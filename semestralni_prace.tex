...
je zapotřebí evidovat údaje o~uživatelích, kanálech, videích, hodnoceních a~aktivitách sledování.

\subsection*{Uživatelé a~autentizace}

O~každém \textbf{uživateli} se eviduje unikátní identifikátor (\texttt{id}, UUID), jméno,
volitelná URL profilového obrázku (\texttt{avatar\_url}) a~role v~rámci platformy --
buď \emph{uzivatel}, nebo \emph{admin}.
...
Tvůrci organizují svůj obsah prostřednictvím \textbf{kanálů}. O~každém kanálu se eviduje
unikátní identifikátor (\texttt{id}, UUID), název a~volitelný slug (URL-přátelský identifikátor,
...
(\texttt{nahledovy\_obrazek\_url}) a~volitelná URL na MPEG-DASH manifest
...
\begin{itemize}
  \item Jaká videa patří do konkrétního kanálu?
  \item Jaké hodnocení má dané video?
  \item Kdo je členem kterého kanálu a~s~jakou rolí?
\end{itemize}
...
\textbf{Poznámka:} Schéma je vytvořeno v~notaci používané na přednáškách. Entitní množiny
jsou znázorněny obdélníky, vztahy kosočtverci, atributy elipsami. Podtržené atributy tvoří
přirozený identifikátor. Hodnocení a~sledování jsou vztahové entity (M:N s~atributy).
...
Následující SQL příkazy byly vygenerovány nástrojem Oracle SQL Developer Data Modeler
(verze 24.3.1) a~doplněny o~příkazy pro definici přístupových práv.
...
(odpovídá na dotaz: \emph{Jaká videa patří do konkrétního kanálu?}):
...
(odpovídá na dotaz: \emph{Jaké hodnocení má dané video?}):
...
(odpovídá na dotaz: \emph{Kdo je členem kterého kanálu a~s~jakou rolí?}):
...
v~INSERT příkazech v~kapitole~5.1. Sloupce obsahující pouze URL adresy
(logo\_url, avatar\_url, nahledovy\_obrazek\_url, manifest\_url apod.) jsou
vynechány ze stejného důvodu.
...
vynechány -- obsahují URL adresy. Kompletní hodnoty viz INSERT příkazy v~kap.~5.1.}
